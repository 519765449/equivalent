       subroutine GTPFL(*) 
         ...
         implicit none
         integer :: err = 0
         integer pflun,si,i,rd1,rd2     !变量定义
        ...
        !---------------------------------
          ...
          do si = 1,size(slun)
            open(slun(si),status = 'scratch')  !用数组slun表示各对应潮流结果
         end do
...
1111    contains
        subroutine RDPBUS !读取pfl中bus data
                implicit none
                integer stat,si
                logical alive
                character(len=220) str , string,string0
                if(ctno(1) == 0) return
                allocate(pbus(ctno(1)))             !type::pbusty动态分配空间到PBUS
                inquire(unit=slun(1),iostat = stat,exist = alive)           !stat为0，alive为true是正确调用
         if(alive)then
                rewind(slun(1))                    !指针移到首端
                do si = 1,ctno(1)
                    read(slun(1),'(A)',iostat = stat) str
                    pbus(si).busname = trim(str(2:9))
                    string0= adjustl(str(13:18)) 
                    read(string0,'(F4.0)') pbus(si).buskv                          !读基准电压
     ...
     subroutine RDPG !读取pfl中generator data
                implicit none
                integer stat,si
                logical alive
                character(len=220) str,string0,string1,string2
                if(ctno(5) == 0) return
                allocate(pg(ctno(5)))             !type::pgty动态分配空间到pg
                inquire(unit=slun(5),iostat = stat,exist = alive)           !stat为0，alive为true是正确调用
         if(alive)then
                rewind(slun(5))                !指针移到首端
                do si = 1,ctno(5)
